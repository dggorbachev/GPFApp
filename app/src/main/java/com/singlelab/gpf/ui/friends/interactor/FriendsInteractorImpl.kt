...
class FriendsInteractorImpl(private val repository: FriendsRepository) : FriendsInteractor,
    ...
    override suspend fun getFriends(personUid: String) =
        repository.getFriends(personUid)?.mapNotNull {
            Person.fromResponse(it)
        }

    override suspend fun search(request: SearchPersonRequest) =
        repository.search(request)?.mapNotNull {
            Person.fromResponse(it)
        }
    ...
    override suspend fun getPersonsFromContacts(phones: List<String>) =
        repository.getPersonsFromContacts(phones)?.mapNotNull {
            Person.fromResponse(it)
        }
}