...
class GridItems(sizeX: Int, sizeY: Int) {
    ...
    init {
        mField = Array(sizeX) { arrayOfNulls<GridTiles>(sizeY) }
        mUndoField = Array(sizeX) { arrayOfNulls<GridTiles>(sizeY) }
        mBufferField = Array(sizeX) { arrayOfNulls<GridTiles>(sizeY) }
        clearGrid()
        clearUndoGrid()
    }
}